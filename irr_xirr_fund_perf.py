...
def find_irr(cashflows):
    ...
    irr = None
    try:
      irr = newton(calculate_npv, x0 = 0.1, fprime=lambda r : 0, args = (cashflows,))
    except:
       irr = np.nan  # if the computation fails
    return irr

# Calculate XIRR using the bisection method
def find_xirr(cashflows, dates, tolerance=1e-7, max_iterations=100):
    low_rate = -0.99  # Ensure to have a negative NPV
    high_rate = 10.0    # Ensure to have a positive NPV
    xirr = None
    try:
       xirr = newton(calculate_xnpv, x0=0.1, fprime=lambda r : 0, args = (cashflows,dates))
    except:
         xirr = np.nan  # if the computation fails
    return xirr
